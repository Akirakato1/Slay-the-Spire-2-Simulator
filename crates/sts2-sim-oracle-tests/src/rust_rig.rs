...
pub fn combat_dump_with_master(cs: &CombatState, master_deck: &[(String, i32)]) -> Value {
    ...
    if let Some(ps) = cs.allies.first().and_then(|c| c.player.as_ref()) {
        if let Some(osty) = ps.osty.as_ref() {
            // C# Necrobinder OstyModel is summoned with a
            // DieForYouPower(amount=1) automatically (PetOwner
            // redirect-damage power). Decorative pets (Byrdpip,
            // PaelsLegion, etc.) do NOT get DieForYouPower — they're
            // visual companions with no redirect mechanic.
            let powers = if osty.osty_id == "Default" {
                json!([{"id": "POWER.DIE_FOR_YOU_POWER", "amount": 1}])
            } else {
                json!([])
            };
            allies.push(json!({
